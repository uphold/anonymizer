...
In certain scenarios, you may want to trim redacted values to keep the output shorter. Such example is f you are redacting logs and sending them to a provider.
...
A flag that enables trimming all redacted values, saving their keys to a `__redacted__` list (default value is `false`). Please note that trimming is only applied when the replacement value is `--REDACTED--`.
